...
def load_model(device="cpu"):
    ...
    try:
        ...
        print("fatal: segmentation model file not found")
        return None
    model.to(device)
    model.eval()
...
def run_full_pipeline(image_np, model, device="cpu"):
    ...
    if len(contours) > 0:
        ...
        ledg_coords = {
            "start": (top_left * [scale_x, scale_y]).astype(int).tolist(),
            "end": (bottom_left * [scale_x, scale_y]).astype(int).tolist(),
        }
        redg_coords = {
            "start": (top_right * [scale_x, scale_y]).astype(int).tolist(),
            "end": (bottom_right * [scale_x, scale_y]).astype(int).tolist(),
        }
